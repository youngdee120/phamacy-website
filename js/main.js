...
createPoductBannerList(freqBookedLabTestList, freqBookedLabTestContainer)
...
createOfferList(wellnessEssentialsList, wellnessEssentialsContainer);

function createPoductBannerList(list, container) {
    list.forEach(function (el) {
        ...
        if (typeof el.discount == "number") {
            ...
            price.innerHTML = "<span>Ksh " + el.mrp + "</span><br>" + "Ksh " + el.price;
        } else {
            discount.innerText = el.discount;
            discount.setAttribute("class", "no-discount");
            price.innerHTML = "Ksh " + el.price;
        }
...
function createProductList(list, container) {
    list.forEach(function (el) {
        ...
        // Convert MRP to Kenyan Shillings
        let mrpInKsh = el.mrp * 1.7;
        mrp.innerHTML = "<span>KSh " + mrpInKsh.toFixed(2) + "</span>";

        // Calculate discounted price in Kenyan Shillings
        let p = (mrpInKsh / 100) * (100 - el.discount);
        p = (Math.round(p * 100) / 100).toFixed(2);
        price.innerHTML = "KSh " + p + "<span> " + el.discount + "% OFF</span>";
